...
void SurfaceMesh::cut_mesh(std::vector<pmp::Halfedge>& hs)
{
    std::vector<pmp::Halfedge> newedges;
    for (int i = 0; i < hs.size(); i++) {
        ...
        auto v0 = to_vertex(hs[i0]);
        auto v1 = to_vertex(hs[i1]);
        auto h0 = new_edge(v0, v1);
        newedges.push_back(h0);

        set_next_halfedge(h0, next_halfedge(h));
        set_next_halfedge(prev_halfedge(h), h0);

        auto fn = face(next_halfedge(h));
        set_face(h0, fn);
        set_halfedge(fn, h0);
    }

    auto f0 = new_face();
    set_halfedge(f0, hs[0]);
    for (int i = 0; i < hs.size(); i++) {
        auto i0 = (hs.size() - 1 + i) % hs.size();
        auto i1 = i;

        set_next_halfedge(hs[i0], hs[i1]);
        set_face(hs[i0], f0);
    }
    ...
    for (int i = 0; i < newedges.size(); i++)
    {
        auto i0 = (hs.size() - 1 + i) % hs.size();
        auto v0 = to_vertex(hs[i0]);
        set_halfedge(v0, opposite_halfedge(hs[i0]));

        auto newp = position(v0);
        auto newv = add_vertex(newp);

        auto h0 = newedges[i];
        set_halfedge(newv, h0);

        auto crrh = h0;
        for (;;)
        {
            auto h1 = opposite_halfedge(crrh);
            set_vertex(h1, newv);

            if (is_boundary(crrh))
                break;

            crrh = opposite_halfedge(prev_halfedge(crrh));
        }
    }

    auto f1 = new_face();
    set_halfedge(f1, opposite_halfedge(newedges[0]));
    for (int i = 0; i < newedges.size(); i++)
    {
        auto i0 = (hs.size() - 1 + i) % hs.size();
        auto i1 = i;

        auto preh = opposite_halfedge(newedges[i1]);
        auto h = opposite_halfedge(newedges[i0]);

        set_next_halfedge(preh, h);
        set_face(h, f1);
    }
